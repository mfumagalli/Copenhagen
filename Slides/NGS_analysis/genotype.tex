...
\begin{equation}
p(\vec{\theta}|\vec{y}) = \frac{f(\vec{y}|\vec{\theta})\pi(\vec{\theta})}{m(\vec{y})} = \frac{f(\vec{y}|\vec{\theta})\pi(\vec{\theta})}{\int f(\vec{y}|\vec{\theta}) \pi(\vec{\theta}) d\vec{\theta}}
\end{equation}
\end{block}
...
\frametitle{Genotype posterior probability}
	...
	A\\A\\A\\G\\$\epsilon=0.01$\\A,G alleles
